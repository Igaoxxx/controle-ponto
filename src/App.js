...
const TimesheetControl = () => {
  ...
  const [darkMode, setDarkMode] = useState(true); // Tema noturno ativado por padrão
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const result = await storage.get('timesheet_entries');
        if (result && result.value) setEntries(JSON.parse(result.value));
        
        const goalResult = await storage.get('hours_goal');
        if (goalResult && goalResult.value) setHoursGoal(JSON.parse(goalResult.value));
        
        const darkModeResult = await storage.get('dark_mode');
        if (darkModeResult && darkModeResult.value !== null) {
  ...
  useEffect(() => {
    const saveData = async () => {
      if (entries.length >= 0) {
        await storage.set('timesheet_entries', JSON.stringify(entries));
        await storage.set('hours_goal', JSON.stringify(hoursGoal));
        await storage.set('dark_mode', JSON.stringify(darkMode));
      }
    };
    saveData();
  }, [entries, hoursGoal, darkMode, storage]);

  // Aplicar classe dark ao body
  useEffect(() => {
    if (darkMode) {
  ...
  const calculateHours = (entry, lStart, lEnd, exit, isHalfDay) => {
    if (!entry || !exit) return { worked: 0, overtime: 0 };
    ...
    const lunchMin = isHalfDay ? 0 : (toMin(lEnd) - toMin(lStart));
    const workedMin = totalMin - lunchMin;
    
    return workedMin / 60;
  };

  const handleSuggestedExit = (entry, date, isHalfDay) => {
    if (!entry || isHalfDay) { setSuggestedExit(''); return; }
  ...
  // --- FUNÇÕES NOVAS PARA HORAS A PAGAR ---
  const updateHoursPaid = (hoursToAdd) => {
    setHoursGoal(prev => ({
      ...prev,
      hoursPaid: Math.max(0, prev.hoursPaid + hoursToAdd)
    }));
  };
  ...
  // const percentage = getProgressPercentage();
  //if (percentage >= 100) return 'bg-emerald-900/30';
  // if (percentage >= 70) return 'bg-amber-900/30';
  ...
  const addEntry = () => {
    ...
    let worked = 0;
    if (!currentEntry.isAbsence) {
      worked = calculateHours(currentEntry.entry, currentEntry.lunchStart, currentEntry.lunchEnd, currentEntry.exit, currentEntry.isHalfDay);
    }
    ...
    const newEntry = {
      ...
      exceptional: new Date(currentEntry.date) <= new Date('2026-01-31')
    };
    ...
    setCurrentEntry({ date: new Date().toISOString().split('T')[0], entry: '', lunchStart: '12:00', lunchEnd: '13:00', exit: '', isHalfDay: false, isAbsence: false });
    setSuggestedExit('');
  };
  ...
  const summary = useMemo(() => {
    ...
    const currentWeekHours = entries.slice(-5).reduce((sum, e) => sum + e.workedHours, 0);

    return { ...total, currentWeekHours, previousWeekHours: 0, compensatedHours: total.overtime };
  }, [entries]);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className={`p-6 rounded-2xl shadow-lg ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-slate-200'} border transition-colors duration-300 flex flex-col md:flex-row justify-between items-center gap-4`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-slate-500'}`}>
                {saveStatus || 'Sincronizado com navegador'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button 
              onClick={() => {
                setSaveStatus('↻ Atualizando visualização...');
                setTimeout(() => setSaveStatus('✓ Visualização atualizada'), 1000);
              }} 
        ...
        {/* SEÇÃO: HORAS A PAGAR */}
        <div className={`p-6 rounded-2xl shadow-lg border transition-colors duration-300 ${darkMode ? 'bg-gradient-to-r from-gray-800 to-gray-900 border-gray-700' : 'bg-gradient-to-r from-indigo-50 to-blue-50 border-indigo-100'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-sm ${darkMode ? 'text-gray-500' : 'text-slate-500'}`}>Ex: 10</span>
                <input 
                  type="number" 
                  step="0.5"
                  min="0"
                  ...
                  onChange={e => setHoursGoal({...hoursGoal, total: parseFloat(e.target.value) || 0})}
                  placeholder="0"
                />
                ...
                {entries.map((entry, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button 
                        onClick={() => { setEditingIndex(index); setCurrentEntry(entries[index]); }} 
        ...
        <div className={`rounded-xl p-4 border transition-colors duration-300 ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-slate-50 border-slate-200'}`}>
          <div className="flex items-center gap-2 text-sm">
            <div className={`w-2 h-2 rounded-full animate-pulse ${darkMode ? 'bg-emerald-500' : 'bg-emerald-400'}`}></div>
            ...
            <span className={darkMode ? 'text-gray-400' : 'text-slate-500'}>
              Seus dados são salvos automaticamente a cada alteração e ficam armazenados no seu navegador.
              Você pode fechar esta página e voltar quando quiser - seus registros estarão aqui!
            </span>
          </div>
        </div>
